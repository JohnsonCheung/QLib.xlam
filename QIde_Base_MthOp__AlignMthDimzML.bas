...
End Function
Friend Function Ccls(NoSf As Boolean, Md As CodeModule, MlNm$) As CodeModule
If NoSf Then Exit Function
Set Ccls = MdzPN(PjzM(Md), WCclsNm(Md, MlNm))
End Function
...
Friend Function OEnsSf(NoSf As Boolean, Md As CodeModule, MthLno&, MlNm$) As Unt
If NoSf Then Exit Function
Dim Lno&: Lno = SrcLnozNxt(Md, MthLno)
Dim OldL$: OldL = Md.Lines(Lno, 1)
Dim NewL$: NewL = "Static F As New " & WCclsNm(Md, MlNm)
If OldL <> NewL Then
    Stop
...
Friend Function OEnsCcls(NoSf As Boolean, Md As CodeModule, MlNm$) As Unt
'Ret #Ens-Chd-Cls.
If NoSf Then Exit Function
EnsCmpzPTN PjzM(Md), vbext_ct_ClassModule, WCclsNm(Md, MlNm)
End Function
...
Friend Function CrAct(CmEpt$(), CmMd As CodeModule) As Drs
