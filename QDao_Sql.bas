Attribute VB_Name = "QDao_Sql"
Option Explicit
Private Const Asm$ = "QDao"
Private Const CMod$ = "MDao_Sql."
Public Const DoczNN$ = "It a Str or Sy will give Ny.  See TermSy"
Const KwBet$ = "between"
Const KwUpd$ = "update"
Const KwInto$ = "into"
Const KwSel$ = "select"
...
AddPfxNLTT = Jn(AddPfxzSy(Sy, C_NLTT), "")
End Function
...
SqpGp_ExprVblAy = "|  Group By " & JnCrLf(FmtExprVblAy(ExprVblAy))
End Function
...
Dim P1$()
    Dim J%, M$
    For J = 0 To UB(Fny)
        If UCase(Fny(J)) = UCase(ExtNy(J)) Or ExtNy(J) = "" Then
            M = ""
        Else
            M = QuoteSq(ExtNy(J))
        End If
    ...
    End If
Dim P2$(): If FmtSql Then P2 = AlignLzSy(Fny) Else P2 = Fny
SqpSelX_Fny_ExtNy = KwSel & C_T & JnComma(AddPfxzSy(JnAyab(P1, P2), C_NLTT))
End Function
...
JnCommaSpcFF = JnCommaSpc(TermSy(FF))
End Function

Function SqpSel_FF$(FF$, Optional Dis As Boolean)
SqpSel_FF = SqpSel_Fny(SyzSsLin(FF), Dis)
End Function
...
Fny = TermSy("[A xx] B C"): Vy = Array(1, "2", #12/1/2018 12:34:56 PM#): GoSub Tst
Exit Sub
Tst:
...
Dim Ey$(): Ey = SyzDicKy(EDic, TermSy(FF))
...
Dim O$(): O = SyzSsLin(OrdMinusSfxFF)
Dim I, J%
For Each I In O
    If HasSfx(O(J), "-") Then
        O(J) = RmvSfx(O(J), "-") & " desc"
...
FnyAlignQuote = AlignLzSy(SyQuoteSq(Fny))
End Function
