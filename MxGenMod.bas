...
:             MsgBox "SrcCd saved:" & vbCrLf & WrdStszL(NewL), vbInformation
End Sub
...
:             MsgBox "SrcCd loaded:" & vbCrLf & WrdSts(SrcCd), vbInformation
End Sub
...
:                                        DltLoRow Lo                                                               ' <== Delete
:                                        PutAyV SrcCd, A1zLo(Lo)                                                   ' <== Load
:                                        MsgBox "SrcCd generated:" & vbCrLf & WrdSts(SrcCd), vbInformation
End Sub
...
Dim Dcl$()
    ...
    For J = 0 To U
        ...
        Case Else: Thw CSub, "Invalid StpTy.  Should be Sub|Fun|Stmt|Expr", "[StpTy with err] RowIx MBdy", StpTy, J, FmtCellDrs(MBdy)
        End Select
    Next
Dim LHS$()
    ReDim LHS(U)
...
For Each Dr In MBdy.Dy
    ...
    Case "Fun", "Expr"
        ...
        Else
            Callg = Expr
        End If
    Case Else: Thw CSub, "Invalid StpTy.  Should be Sub|Fun|Stmt|Expr", "[StpTy with err] RowIx MBdy", StpTy, Ix, FmtCellDrs(MBdy)
    End Select
