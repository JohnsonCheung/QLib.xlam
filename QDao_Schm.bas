...
Dim DicF As Dictionary: Set DicF = Dic(SywRmvTT(Schm, C_Des, C_Fld))
                   AppTdAy A, T
                   RunSqy A, P
                   RunSqy A, S
...
EFzSchm.EleLy = SywRmvT1(Schm, "Ele")
EFzSchm.FldLy = SywRmvT1(Schm, "Fld")
End Function
...
For Each I In Itr(TdLy)
    ...
    Sk = SkFny(TdLin)
    If Si(Sk) > 0 Then
        PushI SqyCrtSk, SqlCrtSk_T_SkFny(T1(TdLin), RplStarzSy(Sk, T1(TdLin)))
    End If
Next
End Function
