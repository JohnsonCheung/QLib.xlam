...
Dim J%: For J = 0 To SiVr(Pm) - 1
    PushIAy XRmkzPmAy, XRmkzPm(Pm(J), WV, W1, W2)
Next
End Function

Private Function XRmkzPm(Pm As Vr, WV%, W1%, W2%) As String()
Dim J%: For J = 0 To SiR123(Pm.R) - 1
    Dim F$: F = IIf(J = 0, "Fm " & Pm.V, "")
    PushI XRmkzPm, XRmkzLin(F, Pm.R(J), WV, W1, W2)
Next
End Function

Private Function XRmkzLin$(F$, R As R123, WV%, W1%, W2%)
Dim R0$, R1$, R2$
If Left(R.R1, 3) <> " ' " Then Stop
R0 = "'" & AlignL(F, WV + 3)
R1 = Mid(AlignL(R.R1, W1), 4)
R2 = AlignL(R.R2, W2)
...
Dim J%: For J = 0 To SiR123(Ret) - 1
    Dim F$: F = IIf(J = 0, "Ret", "")
    PushI XRmkzRet, XRmkzLin(F, Ret(J), WV, W1, W2)
Next
End Function

Private Function XRmkL$(A As Vpr)
Dim Vy$():    Vy = XVyzPm(A.Pm) ' All parameter name
Dim WV%:      WV = WdtzAy(Vy)       ' Wdt of all parameter
Dim W1%, W2%:      XW12 A, W1, W2
Dim R1$():    R1 = XRmkzPmAy(A.Pm, WV, W1%, W2%)
Dim R2$():    R2 = XRmkzRet(A.Ret, WV, W1%, W2%)
           XRmkL = RTrim(JnCrLf(SyzAdd(R1, R2))) & " @@"
End Function

Friend Function CrRmkV(CrVprAy() As Vpr) As S1S2s
'Fm CrVprAy : Sam itm as CrPm
'Ret        : V RmkLines @@
Dim J%: For J = 0 To SiVpr(CrVprAy) - 1
    PushS1S2 CrRmkV, S1S2(CrVprAy(J).V, XRmkL(CrVprAy(J)))
Next
'Insp "QIde_B_MthOp.CrRmkV", "Inspect", "Oup(CrRmkV) CrVprAy", FmtS1S2s(CrRmkV), "NoFmtr(() As Vpr)": Stop
End Function
...
Friend Function CrVprAy(CrPm As Drs, CrVrAy() As Vr) As Vpr()
'Fm CrPm : V Pm
'Ret     : Sam itm as CrPm @@
...
Dim Dr: For Each Dr In Itr(CrPm.Dry)
    ...
    PushVpr CrVprAy, M
Next
If NReczDrs(CrPm) <> SiVpr(CrVprAy) Then Stop
'Insp "QIde_B_MthOp.CrVprAy", "Inspect", "Oup(CrVprAy) CrPm CrVrAy", "NoFmtr(Vpr())", FmtDrs(CrPm), "NoFmtr(() As Vr)": Stop
End Function
...
Friend Function CrVrAy(CrWiRmk As Drs) As Vr()
'Fm CrWiRmk : V R1 R2 R3 ! rmv those R1 2 3 are blank @@
Dim IxV%, IxR1%, IxR2%, IxR3%
AsgIx CrWiRmk, "V R1 R2 R3", IxV, IxR1, IxR2, IxR3
...
Next
PushVr CrVrAy, V, M
'Insp "QIde_B_MthOp.CrVrAy", "Inspect", "Oup(CrVrAy) CrWiRmk", "NoFmtr(Vr())", FmtDrs(CrWiRmk): Stop
End Function
