...
'Insp "QIde_Ens_EnsSubZ.XDiArgNmqArgVar", "Inspect", "Oup(XDiArgNmqArgVar) MD2", XDiArgNmqArgVar, FmtDrs(MD2): Stop
End Function
...
'':MN: :Mthn:
'':AG: :Arg:
Dim Md       As Drs:       Md = AddColoMthPm(DoMth(M))                                                        ' :Drs-L Mdy Ty Mthn MthLin>
Dim MdRetObj As Drs: MdRetObj = SelDrs(AddColoIsRetObj(AddColzRetAs(Md)), "Mdy Ty Mthn MthPm RetSfx IsRetObj") ' :Drs-Mdy Ty Mthn MthPm RetSfx IsRetObj>
...
'Insp "QIde_Ens_EnsSubZ.XMDCallPm", "Inspect", "Oup(XMDCallPm) MdNoZ D", FmtDrs(XMDCallPm), FmtDrs(MdNoZ), "NoFmtr(DiArgItmqVar)": Stop
End Function
...
'Insp "QIde_Ens_EnsSubZ.XMDCall", "Inspect", "Oup(XMDCall) MdCallPm", FmtDrs(XMDCall), FmtDrs(MdCallPm): Stop
End Function
