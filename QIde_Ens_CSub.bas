Attribute VB_Name = "QIde_Ens_CSub"
Option Explicit
...
Type MthRgs1: N As Integer: Ay() As MthRg: End Type
...
Private Function ActLin(Act As EmLinAct, Lno&, Lin$) As ActLin
...
If ShouldIns(IsUsing, OldLin, NewLin) Then PushObj CModAct, ActLin(EiInsLin, InsLno, NewLin)
If ShouldDlt(IsUsing, OldLin, NewLin) Then PushObj CModAct, ActLin(EiDltLin, Lno, OldLin)
End Function
...
If ShouldIns(IsUsing, OldLin, NewLin) Then PushObj CSubActzSng, ActLin(EiInsLin, InsLno, NewLin)
If ShouldDlt(IsUsing, OldLin, NewLin) Then PushObj CSubActzSng, ActLin(EiDltLin, Lno, OldLin)
