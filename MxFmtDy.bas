...
Private Function ColRgAyzCellDr(CellDr$()) As Variant()
Dim Cell: For Each Cell In CellDr
    PushI ColRgAyzCellDr, SplitCrLf(Cell)
Next
End Function

Private Function SqzCellDr(CellDr$()) As Variant()
Dim NRow%: NRow = NRowzCellDr(CellDr)
Dim NCol%: NCol = Si(CellDr)
Dim ColAy(): ColAy = ColRgAyzCellDr(CellDr)
...
Private Function FmtCellDr$(CellDr$(), W%(), S$, Q$)
'@S :SepChr
'@Q :QteChr
Dim Sq(): Sq = SqzCellDr(CellDr)
