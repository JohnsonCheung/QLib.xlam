...
Dim S$
    S = JnSpc(SyQuoteSqIf(RplStarzSy(Sk, T)))
    If S <> "" Then S = " " & S & " |"
...
JnCommaSpcFF = JnQSqCommaSpc(TermSy(FF))
End Function
...
ValzTFRecId = ValzQ(A, SqlSel_FF_T(T, F, BexprRecId(T, RecId)))
End Function
