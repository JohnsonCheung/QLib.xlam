Attribute VB_Name = "QTp_SqyRslt_4_SqyRsltzGpAy"
Option Explicit
Private Const CMod$ = "MTp_SqyRslt_4_SqyRsltzGpAy."
...
Dim O As SqlRslt
    Select Case Ty
    Case EiUpdStmt: O = SqlRsltUpd(NoExprLinSqLy, E)
    Case EiDrpStmt: O = SqlRsltDrp(NoExprLinSqLy)
    Case EiSelStmt: O = SqlRsltSel(NoExprLinSqLy, E)
    Case Else: Stop
    End Select
SqlRsltzSqLy = O
End Function
...
#End If
Dim O$(): O = AddAyAp(Sel, Into$, Fm, Jn, Wh, Gp)
...
Private Function StmtTy(SqLy$()) As EmStmtTy
Dim L$
L = UCase(RmvPfx(T1(SqLy(0)), "?"))
Select Case L
Case "SEL": StmtTy = EiSelStmt
...
Stop
Select Case Ty
Case EiSelStmt: StmtSwKey = StmtSwKey_SEL(SqLy)
Case EiUpdStmt: StmtSwKey = StmtSwKey_UPD(SqLy)
Case Else: Stop
End Select
End Function
...
PushI Ly, "B B0"
D.RemoveAll
    D.Add "A", JnCrLf(SyzSsLin("B0 B1 B2"))
    D.Add "B", "B0"
    Set Ept = D
GoSub Tst
Exit Sub
