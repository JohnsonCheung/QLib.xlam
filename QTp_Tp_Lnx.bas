Attribute VB_Name = "QTp_Tp_Lnx"
Option Explicit
Private Const CMod$ = "MTp_Tp_Lnx."
...
DupT2AyzLnxAy = AywDup(T2Sy(LyzLnxAy(A)))
End Function
Function LyzLnxAy(A() As Lnx) As String()
LyzLnxAy = SyzOyPrp(A, "Lin")
End Function

Function LnxSyeT1Sy(A() As Lnx, T1Sy$()) As Lnx()
Dim L
For Each L In A
    If Not HasEle(T1Sy, T1(CvLnx(L).Lin)) Then PushObj LnxSyeT1Sy, L
Next
End Function
...
For Each I In Itr(A)
    ...
    End With
Next
LyzLnxAyzWithLno = AlignzBySepss(O, ":")
End Function

Function ErzLnxAyT1ss(A() As Lnx, T1ss$) As String()
Dim T1Sy$(): T1Sy = SyzSsLin(T1ss)
If Si(T1Sy) = 0 Then Exit Function
Dim Er() As Lnx: Er = LnxSyeT1Sy(A, T1Sy)
If Si(Er) = 0 Then Exit Function
ErzLnxAyT1ss = LyzMsgNap("There are lines have invalid T1", "Lines Valid-Ty", LyzLnxAyzWithLno(Er), T1Sy)
End Function
...
Function LnxSywRmvT1(A() As Lnx, T$) As Lnx()
Dim O()  As Lnx, X
For Each X In Itr(A)
    With CvLnx(X)
        If T1(.Lin) = T Then
