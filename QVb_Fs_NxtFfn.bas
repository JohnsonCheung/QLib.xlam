...
End Function
Function NxtFfnzNotIn(Ffn$, NotInFfnSy$())
Dim J%, O$
O = Ffn
While HasEleS(NotInFfnSy, O)
...
Function NxtFfnSy(Ffn$) As String() 'Return ffn and all it nxt ffn in the pth of given ffn
If HasFfn(Ffn) Then Push NxtFfnSy, Ffn  '<==
Dim A$()
    Dim Spec$
        Spec = AddFnSfx(Fn(Ffn), "(???)")
    A = FfnSy(Pth(Ffn), Spec)
Dim I, F$
For Each I In Itr(A)
    F = I
    If IsNxtFfn(Ffn) Then PushI NxtFfnSy, F   '<==
Next
End Function
