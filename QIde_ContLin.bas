...
Tst:
    ...
    Return
End Sub
Function ContLinCntzMd%(A As CodeModule, Lno&)
Dim J&, O%
For J = Lno To A.CountOfLines
    ...
    End If
Next
Thw CSub, "LasLin of Md cannot be end of [_]", "LasLin-Of-Md Md", A.Lines(A.CountOfLines, 1), MdNm(A)
End Function
...
Function ContLin$(Src$(), Optional Ix& = 0, Optional OneLin As Boolean)
If OneLin Then
...
Function IsInMdLno(A As CodeModule, Lno&) As Boolean
IsInMdLno = BetLng(Lno, 1, A.CountOfLines)
End Function

Private Function ContToLno&(A As CodeModule, Lno&)
If Not IsInMdLno(A, Lno) Then Exit Function
Dim J&
For J = Lno To A.CountOfLines
