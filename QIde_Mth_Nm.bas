Attribute VB_Name = "QIde_Mth_Nm"
Option Explicit
Private Const Asm$ = "QIde"
Private Const CMod$ = "MIde_Mth_Nm."
Type MdMth
    Md As CodeModule
    MthNm As String
End Type
...
Function MthNyzMthLinSy(MthLinSy$()) As String()
Const CSub$ = CMod & "MthNyzMthLinSy"
Dim I, Nm$, J%, MthLin$
For Each I In Itr(MthLinSy)
    MthLin = I
    Nm = MthNm(MthLin)
    If Nm = "" Then Thw CSub, "Given MthLinSy does not have MthNm", "[MthLin with error] Ix MthLinSy", I, J, AddIxPfx(MthLinSy)
    PushI MthNyzMthLinSy, Nm
    J = J + 1
Next
End Function
...
RmvMthMdy = RmvTermSy(Lin, MthMdyAy)
End Function
