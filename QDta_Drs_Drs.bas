...
Dim O()
    ...
    Dim Rxy: For Each Rxy In Itr(G)         ' Rxy is gp-of-rix-poiting-to-@D-row
        ...
        Dim Rix: For Each Rix In Rxy        ' Rix is rix-poiting-to-@D-row
            Push ODy, Dy(Rix)           ' Push the @D-row to @ODy
        Next
...
'Ret   : ..@Gpcc..@C.. ! where @C is opt gp of col-C, in :Av: @@
If C = "" Then
    Agr = SelDist(D, Gpcc)
    Exit Function
End If
...
Dim G(): G = Gp(D, Gpcc)          ' each gp (an ele of #G) is a dry-of-@D with sam @Gpcc col val
...
End With
End Function
Function DrszNewDy(A As Drs, NewDy()) As Drs
With DrszNewDy
    .Fny = A.Fny
    .Dy = NewDy
End With
