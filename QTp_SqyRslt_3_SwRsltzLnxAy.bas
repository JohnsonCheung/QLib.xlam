Attribute VB_Name = "QTp_SqyRslt_3_SwRsltzLnxAy"
Option Explicit
Private Const Asm$ = "QTp"
Private Const CMod$ = "MTp_SqyRslt_3_SwRsltzLnxAy."
...
Stop
Select Case A.OpStr
Case "OR":  R = EvlTermSy(Ay, "OR", Sw, BoolTermPm)
Case "AND": R = EvlTermSy(Ay, "AND", Sw, BoolTermPm)
...
Private Function EvlTermSy1(SwTermSy$(), AND_OR$, Sw As Dictionary, BoolTermPm As Dictionary) As Boolean()
Dim R As BoolOpt, O() As Boolean, I
For Each I In SwTermSy
...
Private Function EvlTermSy(SwTermSy$(), AND_OR$, Sw As Dictionary, BoolTermPm As Dictionary) As BoolOpt
Dim O As Boolean
If Si(SwTermSy) = 0 Then Stop
Dim BoolAy() As Boolean
    BoolAy = EvlTermSy1(SwTermSy, AND_OR, Sw, BoolTermPm)
...
Static X$()
If Si(X) = 0 Then X = SyzSsLin("OR AND NE EQ")
OpStrAy = X
End Property
