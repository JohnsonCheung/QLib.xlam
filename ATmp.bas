...
Private Function AddColzF0(A As Drs) As Drs
Dim IxMcLin%: IxMcLin = IxzAy(A.Fny, "McLin", ThwEr:=EiThwEr)
Dim IxV%:         IxV = IxzAy(A.Fny, "V")
Dim Dr:            Dr = A.Dy(0)
Dim V$:             V = Dr(IxV)
Dim McLin$:     McLin = Dr(IxMcLin)
Dim T$:             T = LTrim(McLin)
Dim F0%:           F0 = IIf(V = "", 0, Len(McLin) - Len(T))
                  AddColzF0 = AddCol(A, "F0", F0)
End Function
...
'Fm Mc : L MthLin # Mth-Context.
'Ret   : L McLin # Mc-Cln.      ! :McLin: is &XIsLinMc
Dim Dr, Dy()
For Each Dr In Itr(Mc.Dy)
    If XIsLinMc(Dr(1)) Then PushI Dy, Dr
Next
...
End Function
Private Function XAvoLRaColonaRst(Lin$) As Variant()
Dim LHS$, RHS$, IsColon As Boolean, Rst$
    ...
    Else
        ...
        End If
    End If
XAvoLRaColonaRst = Array(LHS, RHS, IsColon, Rst)
End Function
...
Dim Dr: For Each Dr In Itr(McDcl.Dy)
    Dim L$:    L = Pop(Dr)
    Dim Av(): Av = XAvoLRaColonaRst(L) ' LHR RHS IsColon Rst
