Attribute VB_Name = "MTp_SqyRslt_32_SwBrkAyRslt"
Option Explicit
...
Dim Ok3() As SwBrk
Dim Ok4() As SwBrk
Dim Er$(): Er = AyAddAp(ErzLin(A, Ok1), ErzDupNm(Ok1, Ok2), ErzFld(Ok2, Ok3), ErzLeftOvr(Ok3, Ok4))
...
Private Function MsgzTermMustBegWithQuestOrAt$(TermAy$(), A As SwBrk)
MsgzTermMustBegWithQuestOrAt = Msgz(A, "Terms[" & JnSpc(TermAy) & "] must begin with either [?] or [@?]")
End Function

Private Function MsgzTermNotInPm$(TermAy$(), A As SwBrk)
MsgzTermNotInPm = Msgz(A, "Terms[" & JnSpc(TermAy) & "] begin with [@?] must be found in Pm")
End Function

Private Function MsgzTermNotInSw$(TermAy$(), A As SwBrk, SwNm As Dictionary)
