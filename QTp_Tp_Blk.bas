...
Type Blk: BlkTy As String: LinzSep As String: Lnxs As Lnxs: End Type
Type Blks: N As Long: Ay() As Blk: End Type
Function Blks(Tp$, Optional LinzSepPfx$ = "==") As Blks
If Trim(Tp) = "" Then Exit Function
Dim M As Blk, L
Dim InBlk As Boolean, J&, IsBrk As Boolean
For Each L In RmvRmkzLy(SplitCrLf(Tp))
    IsBrk = HasPfx(L, LinzSepPfx)
    Select Case True
    Case IsBrk: PushBlk Blks, M: M = BlkzLinzSep(L)
...
If A.LinzSep <> "" Then Exit Function
'If Not IsLnxsOfEmp(A.Lnxs) Then Exit Function
IsBlkOfEmp = True
End Function
Function BlkzLinzSep(LinzSep) As Blk
BlkzLinzSep.LinzSep = LinzSep
End Function
...
PushI FmtBlk, P & Qte(A.BlkTy, "BlkTy(*)")
PushI FmtBlk, A.LinzSep
